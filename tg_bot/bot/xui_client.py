...
class XUIClient:
    ...
    async def get_client_traffic_bytes(self, client_uuid: str, client_email: str) -> tuple[int, int] | None:
        """Читает статистику up/down из ответа inbound (clientStats в 3x-ui)."""
        await self._ensure_login()
        path = self.config.xui_get_inbound_endpoint.format(inbound_id=self.config.xui_inbound_id)
        data = await self._request("GET", path)
        obj = data.get("obj")
        if not isinstance(obj, dict):
            return None
        stats = obj.get("clientStats")
        if not isinstance(stats, list):
            return None
        cu = (client_uuid or "").strip().lower()
        ce = (client_email or "").strip().lower()
        for row in stats:
