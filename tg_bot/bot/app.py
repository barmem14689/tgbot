...
class VPNPaymentBot:
    ...
    async def on_start(self, message: Message) -> None:
        ...
        await message.answer(text, reply_markup=subscription_periods_kb(self.config.subscription_periods))
    ...
    async def on_photo(self, message: Message) -> None:
        ...
        if not payment:
            await message.answer(
                "Сначала выберите период подписки через /start, затем отправьте скриншот оплаты."
            )
            return
        ...
        await message.answer("Скриншот получен и отправлен в админ-панель. Ожидайте подтверждения.")
