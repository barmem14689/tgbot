...
class UserRecord:
    ...
    sub_expiry_warned_for_at: datetime | None
...
class Repository:
    ...
    async def get_user(self, tg_user_id: int) -> UserRecord | None:
        ...
        return UserRecord(
            ...
            sub_expiry_warned_for_at=(
                datetime.fromisoformat(row["sub_expiry_warned_for_at"])
                if row["sub_expiry_warned_for_at"]
                else None
            ),
        )
    ...
    async def set_user_subscription(
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                """
                UPDATE users
                SET xui_uuid = ?, xui_email = ?, key_text = ?, subscription_expires_at = ?, sub_expiry_warned_for_at = NULL, updated_at = ?
                WHERE tg_user_id = ?
                """,
    ...
    async def extend_user_subscription(self, tg_user_id: int, bonus_days: int) -> datetime | None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                """
                UPDATE users
                SET subscription_expires_at = ?, sub_expiry_warned_for_at = NULL, updated_at = ?
                WHERE tg_user_id = ?
                """,
    ...
    async def users_for_expiry_warning(self, now: datetime, warn_days: int) -> list[UserRecord]:
        if warn_days < 1:
            return []
        deadline = now + timedelta(days=warn_days)
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute(
                """
    ...
    async def mark_sub_expiry_warned_for(self, tg_user_id: int, expires_at: datetime) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                """
                UPDATE users
                SET sub_expiry_warned_for_at = ?, updated_at = ?
                WHERE tg_user_id = ?
                """,
                (expires_at.isoformat(), utc_now_iso(), tg_user_id),
            )
            await db.commit()
