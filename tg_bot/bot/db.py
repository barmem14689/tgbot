...
async def init_db(database_path: str) -> None:
    async with aiosqlite.connect(database_path) as db:
        ...
        if "sub_expiry_warned_for_at" not in column_names:
            await db.execute("ALTER TABLE users ADD COLUMN sub_expiry_warned_for_at TEXT")
        await db.commit()
